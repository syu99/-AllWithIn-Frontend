...
function Signup() {
  ...
  useEffect(() => {
    fetch("https://all-with-in-backend.vercel.app/users/jobs")
  ...
  const confirmClick = (e) => {
    ...
    let token;
    let uploadImg = false;
    fetch("https://all-with-in-backend.vercel.app/users/signup", {
      method: "POST",
      ...
      .then((data) => {
        ...
        if (data.result) {
          if (inputCVRef.current.files[0]) {
            ...
            fetch("https://all-with-in-backend.vercel.app/users/updateCV", {
              method: "POST",
              body: formData,
            })
              .then((res) => res.json())
          ...
          if (inputPhotoRef.current.files[0]) {
            ...
            formData.append("data", JSON.stringify(dataInfo));
            fetch("https://all-with-in-backend.vercel.app/users/updateAvatar", {
              method: "POST",
              body: formData,
            })
              .then((res) => res.json())
