...
function Join() {
  ...
  useEffect(() => {
    ...
    fetch(`https://all-with-in-backend.vercel.app/offers/allOffers`)
      .then((response) => response.json())
      .then((data) => {
    ...
    fetch(`https://all-with-in-backend.vercel.app/jobs`)
      .then((response) => response.json())
      .then((data) => {
  ...
  const handleApplyBtn = (selectProject) => {
    fetch("https://all-with-in-backend.vercel.app/offers/addUserIdOnOffer", {
      method: "POST",
