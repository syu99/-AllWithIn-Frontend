...
function DashboardModal(props) {
  ...
  const handleRefuseClick = (id, users) => {
    fetch("https://all-with-in-backend.vercel.app/offers/confirm", {
      method: "PUT",
      headers: {
  ...
  const handleAcceptClick = (id, users, projectId) => {
    fetch("https://all-with-in-backend.vercel.app/offers/confirm", {
      ...
      .then(
        fetch("https://all-with-in-backend.vercel.app/projects/addCrew", {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        people.map((data) => (
          <div className={styles.card}>
            ...
            <FontAwesomeIcon
              ...
              icon={faSquareCheck}
              onClick={() =>
                handleAcceptClick(props.data._id, data, props.data.project)
              }
            />
            <FontAwesomeIcon
              className={styles.refuse}
              icon={faSquareXmark}
              onClick={() => handleRefuseClick(props.data._id, data)}
            />
          </div>
        ))
      )}
